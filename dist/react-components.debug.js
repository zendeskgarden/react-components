    ...
    function(module, exports) {
        function createIsHidden() {
            ...
            return function isHidden(node) {
                ...
                for (var i = 0, length = nodeCache.length; i < length; i++) if (nodeCache[i][0] === node) return nodeCache[i][1];
                ...
                nodeCache.push([ node, result ]), result;
            };
        }
        module.exports = function(el) {
            for (var candidate, candidateIndex, basicTabbables = [], orderedTabbables = [], isHidden = createIsHidden(), candidates = el.querySelectorAll("input, select, a[href], textarea, button, [tabindex]"), i = 0, l = candidates.length; i < l; i++) candidate = candidates[i], 
            candidateIndex = candidate.tabIndex, candidateIndex < 0 || "INPUT" === candidate.tagName && "hidden" === candidate.type || candidate.disabled || isHidden(candidate) || (0 === candidateIndex ? basicTabbables.push(candidate) : orderedTabbables.push({
