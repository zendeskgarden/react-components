...
expect.addAssertion('<object> to be margin away from one side of <object>', (expect, subject, anchor) => {
  expect((
...
expect.addAssertion('<object> to have the same size as <object>', (expect, subject, value) => {
...
expect.addAssertion('<object> to be inside viewport <object>', (expect, subject, viewport) => {
  expect(isInsideViewport({
    target: subject,
    viewport
  }), 'to be true')
})

expect.addAssertion('<object> to have been moved inside of viewport <object>', (expect, subject, viewport) => {
  expect((
    subject.top === 0 ||
    subject.left === 0 ||
...
const possiblePositionsWithoutStretching = possiblePositions.filter(p => !p.match(/_stretch$/))
...
describe('positioning', () => {
  describe('positionRelative', () => {
    it('produces a target that always has a one side honoring the margins of the anchor', () => {
      expect((position, anchor, target) => {
        const positionedTarget = positionRelative({
          position, anchor, target
        })

        expect(positionedTarget, 'to be margin away from one side of', anchor)
      }, 'to be valid for all', positions, squares, squares)
    })
    ...
    describe('when the target is not positioned with stretching', () => {
      it('does not change the size of the target', () => {
        ...
        }, 'to be valid for all', positionsWithoutStretching, squares, squares)
      })
    })
  ...
  describe('getBestRelativePlacement', () => {
    const positionList = unique(positions, natural({ min: 1, max: possiblePositions.length }))

    it('produces a target that always has one side honoring the margins of the anchor', () => {
      expect((positions, anchor, target, viewport) => {
        ...
        expect(positionedTarget, 'to satisfy', {
          rect: expect.it('to be margin away from one side of', anchor)
                      .or('to have been moved inside of viewport', viewport)
        })
      }, 'to be valid for all', positionList, squares, squares, viewports)
    })
    ...
    describe('when the target is not positioned with stretching', () => {
      const positionList = unique(positionsWithoutStretching, natural({
        min: 1,
