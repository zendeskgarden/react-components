  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (!isOpen && previousisOpen) {
      if (!this.triggerReference) {
        throw new Error(
          'Accessibility Error: You must apply the `triggerRef` prop to the element that triggers your menu.'
        );
      }
      ...
      <Manager>
        ...
        >
          {({ popperProps, scheduleUpdate }) => {
            ...
            let menu = (
              <MenuWrapper innerRef={popperProps.ref} style={popperProps.style}>
                <FocusJailContainer focusOnMount={false}>
                  {({ getContainerProps: getFocusJailContainerProps, containerRef }) => (
                    <SelectionContainer
                      ...
                      direction="vertical"
                      focusedKey={controlledFocusedKey}
                      selectedKey={undefined} // Enforce controlled strategy
                      defaultFocusedIndex={defaultFocusedIndex}
                      onStateChange={newState => {
                        if (newState && newState.selectedKey) {
                          this.onItemSelected(newState.selectedKey);
