...
<Meta title="Packages/Dropdowns/[patterns]" component={Menu} />
...
an element with `overflow: hidden` / `auto` / `scroll` styles.

Note in this example, that the menu is currently getting cropped. Change the
 `appendToNode` property to "portal" to see the full menu. With the menu open,
 also scroll down to see the menu still following the trigger.
