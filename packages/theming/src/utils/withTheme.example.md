The `withTheme` HOC allows you to interact with the `ThemeProvider` in any child component.

```jsx
const LocalizedDiv = withTheme(({ theme, children }) => (
  <div style={{ direction: theme.rtl ? 'rtl' : 'ltr', border: 'grey solid', padding: 16, marginTop: 16 }}>{children}</div>
));
